...
Write ONE new scenario that is UNIQUE and DIFFERENT from anything above:`;
    ...
    if (!response.ok) {
      ...
      console.error(
        "❌ [Letter] Gemini API error for scenario:",
        response.status,
        errorText
      );
    ...
    if (
      ...
      console.error(
        "❌ [Letter] Unexpected response structure from Gemini:",
        data
      );
    ...
    if (candidate.finishReason === "MAX_TOKENS") {
      console.error("❌ [Letter] Scenario response truncated (MAX_TOKENS).");
      throw new Error("Response truncated");
    }

    if (!candidate.content.parts || !candidate.content.parts[0]) {
      console.error("❌ [Letter] Missing parts in content:", candidate.content);
